...
import { Collapsible as BaseCollapsible } from "~primitives/collapsible";
...
export function Sidebar({ size, className, ...props }: ComponentProps<"aside"> & VariantProps<typeof sidebarVariants>) {
...
export function SidebarLogo({ className, children, ...props }: ComponentProps<"div">) {
	return (
		<div data-slot="sidebar-logo" className={cn("flex items-center gap-2.5 text-lg text-foreground select-none", className)} {...props}>
			{children}
		</div>
	);
}
...
export function SidebarMenu({ className, ...props }: ComponentProps<"nav">) {
	return <nav data-slot="sidebar-menu" className={cn("flex flex-col gap-4.5", className)} {...props} />;
}

export function SidebarList({ className, children, ...props }: ComponentProps<"ul">) {
	return (
		<ul data-slot="sidebar-list" className={cn("flex w-full flex-col gap-0.5", className)} {...props}>
			{children}
		</ul>
	);
}

export function SidebarItem({ className, children }: ComponentProps<"li">) {
	return (
		<li data-slot="sidebar-item" className={cn("group/sidebar-item", "relative flex **:data-[slot=sidebar-submenu]:w-auto", className)}>
			{children}
		</li>
	);
}

export const sidebarItemActionVariants = cva("absolute right-0 z-10 shrink-0 transition-all duration-100", {
	variants: {
		size: {
			default: "size-8.5",
			compact: "size-8",
...
export function SidebarItemAction({
	...
	return (
		<div
			...
			className={cn(
				"absolute right-0 z-10 shrink-0 **:[svg]:size-4",
	...
	return useRender({
		...
		props: {
			...
			className: cn(
				...
				"cursor-pointer text-left text-foreground",
...
export function SidebarGroup({ className, children, ...props }: ComponentProps<"section">) {
	return (
...
export function SidebarGroupTitle({ className, children, ...props }: ComponentProps<"span">) {
	return (
		<span
			data-slot="sidebar-group-title"
			className={cn("inline-flex items-center text-sm font-medium text-dimmed", "**:[svg]:size-3.5", className)}
			{...props}
		>
			{children}
		</span>
...
export function SidebarGroupAction({ className, children, ...props }: ComponentProps<"div">) {
	return (
...
export function SidebarSubmenu({ className, ...props }: ComponentProps<"div">) {
	return <div data-slot="sidebar-submenu" {...props} className={cn("w-full border-l border-border py-0.5", className)} />;
}

export function SidebarCollapsible({ className, ...props }: ComponentProps<typeof BaseCollapsible.Root>) {
	return <BaseCollapsible.Root data-slot="sidebar-collapsible" className={cn("w-full", className)} {...props} />;
}

export function SidebarCollapsibleTrigger({ className, ...props }: ComponentProps<typeof BaseCollapsible.Trigger>) {
	return (
		<BaseCollapsible.Trigger
			data-expandable
			{...props}
			className={cn("**:data-[slot=expandable-indicator]:transition-all", "**:data-[slot=expandable-indicator]:duration-100", className)}
		/>
	);
}

export function SidebarCollapsiblePanel({ className, ...props }: ComponentProps<typeof BaseCollapsible.Panel>) {
	return (
